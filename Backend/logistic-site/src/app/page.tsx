...
export default function LogisticsApp() {
  ...
  case "orders":
    return (
      ...
      <form className="mt-6 space-y-4 animate-slideUp delay-300" onSubmit={(e) => {
        e.preventDefault();
        const form = e.target as HTMLFormElement;
        const orderData: OrderData = {
          pickup: form.pickup.value,
        ...
        <div>
          ...
          <input type="text" name="pickup" placeholder="Enter pickup location" required className="border p-2 rounded w-full"/>
        </div>
        <div>
          <label className="block text-lg font-medium">Destination:</label>
          <input type="text" name="destination" placeholder="Enter destination" required className="border p-2 rounded w-full"/>
        </div>
        <div>
          <label className="block text-lg font-medium">Package Weight (kg):</label>
          <input type="number" name="weight" placeholder="Enter weight" required className="border p-2 rounded w-full"/>
        </div>
        <button type="submit" className="bg-green-500 text-white px-4 py-2 rounded transform transition-transform hover:scale-105 hover:-translate-y-1">
          Place Order
        </button>
      </form>
              ...
              {orders.map((order) => (
                <tr key={order.id} className="text-center">
                  ...
                  <td className="border p-2 space-x-2">
                    <button onClick={() => handleEdit(order)} className="bg-yellow-500 text-white px-4 py-2 rounded"> Edit </button>
                    <button onClick={() => handleDelete(order.id)} className="bg-red-500 text-white px-4 py-2 rounded"> Delete </button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {isEditing && (
          <div className="fixed top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex items-center justify-center animate-fadeIn">
            <div className="bg-white p-6 rounded shadow-lg">
              ...
              <input type="text" value={currentOrder?.pickup || ""} onChange={(e) => setCurrentOrder(currentOrder ? { ...currentOrder, pickup: e.target.value } : null)} className="border p-2 rounded w-full mb-4"/>
              <label className="block mb-2">Destination:</label>
              <input type="text" value={currentOrder?.destination || ""} onChange={(e) => setCurrentOrder(currentOrder ? { ...currentOrder, destination: e.target.value } : null)} className="border p-2 rounded w-full mb-4"/>
              <label className="block mb-2">Package Weight (kg):</label>
              <input type="number" value={currentOrder?.weight || ""} onChange={(e) => setCurrentOrder(currentOrder ? { ...currentOrder, weight: Number(e.target.value) } : null)} className="border p-2 rounded w-full mb-4"/>
              <button onClick={handleUpdate} className="bg-green-500 text-white px-4 py-2 rounded mr-4"> Save Changes </button>
